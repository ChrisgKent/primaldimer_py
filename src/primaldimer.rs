...
impl DebruijnFKmer {
    ...
    pub fn new(end: usize, seqs: Vec<String>) -> Self {
        let starts: Vec<usize> = seqs.iter().map(|s| end - s.len()).collect();
        let start = *starts.iter().min().unwrap();
        let dnaseqs: Vec<DnaString> = seqs.iter().map(|s| DnaString::from_dna_string(s)).collect();
        let encoded_seqs: Vec<Vec<usize>> = dnaseqs
            .iter()
            .map(|s| s.to_bytes().iter().map(|f| *f as usize).collect())
            .collect();
        DebruijnFKmer {
            end,
            seqs: dnaseqs,
...
pub struct DebruijnRKmer {
    ...
    pub _end: usize,
    #[pyo3(get)]
    pub _encodedseqs: Vec<Vec<usize>>,
}
#[pymethods]
impl DebruijnRKmer {
    #[new]
    pub fn new(start: usize, seqs: Vec<String>) -> Self {
        let _ends: Vec<usize> = seqs.iter().map(|s| start + s.len()).collect();
        let _end = *_ends.iter().max().unwrap();
        let dnaseqs: Vec<DnaString> = seqs.iter().map(|s| DnaString::from_dna_string(s)).collect();
        let encoded_seqs: Vec<Vec<usize>> = dnaseqs
            .iter()
            .map(|s| s.to_bytes().iter().map(|f| *f as usize).collect())
...
pub fn do_kmers_interact(fkmer: &DebruijnFKmer, rkmer: &DebruijnRKmer, t: f64) -> bool {
    // Will look for interactions between every seq in pool1 and pool2
    for (s1, s2) in fkmer
        .encodedseqs
        .iter()
        .cartesian_product(rkmer._encodedseqs.iter())
    {
        if does_seq1_extend(&s1, &s2, t) | does_seq1_extend(&s2, &s1, t) {
            return true;
        }
...
#[cfg(test)]
mod tests {
    use super::{calc_nn_thermo, encode_base, NN_SCORES};
    #[test]
